...
void main() {
  ...
  group('GetHomeDataUseCase', () {
    test('returns HomeData when all three repo calls succeed', () async {
      ...
      when(() => repo.getRecentDetections(
              userId: any(named: 'userId'), limit: any(named: 'limit')))
          .thenAnswer((_) async => Result.success(scans));
      when(() => repo.getDailyTrend(
              days: any(named: 'days'), userId: any(named: 'userId')))
    ...
    test('returns error when getFarmStats fails', () async {
      when(() => repo.getFarmStats(userId: any(named: 'userId'))).thenAnswer(
          (_) async => Result.error(const CacheFailure('stats fail')));
      when(() => repo.getRecentDetections(
              userId: any(named: 'userId'), limit: any(named: 'limit')))
          .thenAnswer((_) async => Result.success(scans));
      when(() => repo.getDailyTrend(
    ...
    test('returns error when getRecentDetections fails', () async {
      ...
      when(() => repo.getRecentDetections(
              userId: any(named: 'userId'), limit: any(named: 'limit')))
          .thenAnswer(
              (_) async => Result.error(const CacheFailure('scans fail')));
      when(() => repo.getDailyTrend(
              days: any(named: 'days'), userId: any(named: 'userId')))
          .thenAnswer((_) async => Result.success(trend));
    ...
    test('returns error when getDailyTrend fails', () async {
      ...
      when(() => repo.getDailyTrend(
              days: any(named: 'days'), userId: any(named: 'userId')))
          .thenAnswer(
              (_) async => Result.error(const CacheFailure('trend fail')));
    ...
    test('uses default empty values when repo returns null data', () async {
      when(() => repo.getFarmStats(userId: any(named: 'userId'))).thenAnswer(
          (_) async =>
              Result.success({'total': 0, 'healthy': 0, 'diseased': 0}));
      when(() => repo.getRecentDetections(
              userId: any(named: 'userId'), limit: any(named: 'limit')))
