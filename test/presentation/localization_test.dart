...
void main() {
  test('All four .arb files must have identical key sets and preserve ICU placeholders', () {
    ...
    final twJson = jsonDecode(twFile.readAsStringSync()) as Map<String, dynamic>;
    final eeJson = jsonDecode(eeFile.readAsStringSync()) as Map<String, dynamic>;
    final dagJson = jsonDecode(dagFile.readAsStringSync()) as Map<String, dynamic>;

    // Filter out @@locale keys
    ...
    expect(twDiff, isEmpty, reason: 'Keys in app_tw.arb and app_en.arb mismatch: $twDiff');

    final eeDiff = enKeys.difference(eeKeys).union(eeKeys.difference(enKeys));
    expect(eeDiff, isEmpty, reason: 'Keys in app_ee.arb and app_en.arb mismatch: $eeDiff');

    final dagDiff = enKeys.difference(dagKeys).union(dagKeys.difference(enKeys));
    expect(dagDiff, isEmpty, reason: 'Keys in app_dag.arb and app_en.arb mismatch: $dagDiff');
    ...
    void checkPlaceholders(String lang, Map<String, dynamic> jsonMap) {
      for (final entry in enJson.entries) {
        ...
        final enPlaceholders = placeholderRegex.allMatches(enVal).map((m) => m.group(0)).toSet();
        final locPlaceholders = placeholderRegex.allMatches(locVal).map((m) => m.group(0)).toSet();

        expect(locPlaceholders, equals(enPlaceholders),
            reason: 'ICU placeholders for key "$key" in app_$lang.arb must match English. '
